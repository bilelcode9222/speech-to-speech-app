...
export function LanguageSelector({
  ...
  const source = findLanguage(sourceCode);
  const target = findLanguage(targetCode);

  const select = (code: string) => {
    if (picking === 'source') onChangeSource(code);
    if (picking === 'target') onChangeTarget(code);
    setPicking(null);
  };
  ...
  return (
    <>
      <View style={styles.row}>
        <Pressable style={styles.chip} onPress={() => setPicking('source')} disabled={disabled}>
        ...
        <Pressable style={styles.chip} onPress={() => setPicking('target')} disabled={disabled}>
          <Text style={styles.chipLabel} numberOfLines={1}>
        ...
        <Pressable style={styles.backdrop} onPress={() => setPicking(null)}>
          <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                const active =
                  picking === 'source' ? item.code === sourceCode : item.code === targetCode;
                return (
                  <Pressable style={styles.option} onPress={() => select(item.code)}>
...
function createStyles(colors: Palette) {
  return StyleSheet.create({
    ...
    sheetTitle: { ...type.eyebrow, color: colors.textMuted, marginBottom: spacing.sm },
    option: {
      flexDirection: 'row',
