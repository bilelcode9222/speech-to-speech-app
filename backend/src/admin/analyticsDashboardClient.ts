...
function renderMoney() {
 const s=data.summary,e=data.economics;
 $('moneyStats').innerHTML=metric('Ventes brutes confirmées',money(s.revenueUsd),count(s.charges)+' paiements')+metric('Net après Apple · estimé',money(e.estimatedNetRevenueUsd),'Part conservée : '+percent(e.storeNetRevenueShare))+metric('Coût IA estimé',money(e.technicalCostUsd,true),count(e.costEvents)+' calculs reçus')+metric('Marge estimée',money(e.estimatedMarginUsd),'Après IA et acquisition estimée');
 const row=(label,value,extra,total)=>'<div class="money-row'+(total?' total':'')+'"><div>'+label+(extra?'<p class="tiny muted">'+esc(extra)+'</p>':'')+'</div><strong class="'+(value<0?'money-negative':'')+'">'+money(value,true)+'</strong></div>';
 $('moneyBreakdown').innerHTML=row('Ventes brutes',e.grossRevenueUsd,'Achats et renouvellements en production')+row('Part Apple · estimée',e.estimatedNetRevenueUsd-e.grossRevenueUsd,percent(1-e.storeNetRevenueShare)+' des ventes')+row('Net estimé',e.estimatedNetRevenueUsd,'',true)+row('Coût technique IA',-e.technicalCostUsd,'Transcription, traduction et voix mesurées')+row('Acquisition · estimée',-e.acquisitionCostUsd,count(e.newUsers)+' nouveaux utilisateurs × '+money(e.acquisitionCostPerUserUsd))+row('Marge estimée',e.estimatedMarginUsd,'Avant autres frais et ajustements',true);
 ...
 $('moneyRows').innerHTML=data.customers.map(c=>'<tr><td>'+personButton(c)+'</td><td>'+money(c.grossRevenueUsd)+'</td><td>'+money(c.estimatedNetRevenueUsd)+'</td><td>'+money(c.technicalCostUsd,true)+(c.incompleteCostEvents?'<div class="tiny muted">Coût partiel</div>':'')+'</td><td>'+money(c.acquisitionCostUsd)+'</td><td class="'+(c.estimatedMarginUsd<0?'money-negative':'')+'">'+money(c.estimatedMarginUsd,true)+'</td></tr>').join('') || '<tr><td colspan="6"><div class="empty">Aucune donnée financière par utilisateur pour cette période.</div></td></tr>';
}
...
function renderInsights() {
 ...
 if(s.failures)items.push([count(s.failures)+' erreurs de traduction à examiner','Ouvrez Qualité de service pour identifier les étapes concernées.','warn']);
